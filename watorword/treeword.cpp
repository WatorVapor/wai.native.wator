...
bool PredictionTree::setWordSeq(const multimap<int,WordElement> &clearWordSeq) {
   ...
   for(auto keyIt:clearWordSeq) {
    ...
    auto indexElem = std::make_tuple(word,pos,range,weight,weight_orig,uniqIndex_);
...
bool PredictionTree::setWordSeq(const multimap<int,WordElement> &indexWordSeq,int pos){
...
void PredictionTree::splitComplexWordSeq(const multimap<int,WordElement> &clearWordSeq,vector<multimap<int,WordElement>> &splitedSeq) {
...
void PredictionTree::flatPredSeq(multimap<int,vector<WordElement>> &flat,double &weightSum) {
  if(leaves_.size() >iConstLeavesWidthMax) {
     return;
  }
...
//    std::cout << "################" << std::endl;
    ...
    if(dummy.second.empty() == false) {
      ...
      double divPredWeight2 = ((double)(UINT16_MAX) * divPredWeight)/weightMaxDummy;
...
void PredictionTree::flatPredSeqInside(multimap<int,vector<WordElement>> &flat,double &weightMax) {
