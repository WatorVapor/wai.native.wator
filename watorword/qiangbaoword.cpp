...
pt::ptree QiangbaoWord::cut(const vector<string> &wordBytes,const string &text,double &pred) {
  pt::ptree result;
  if(wordBytes.size() <2) {
    return result;
  }
...
void QiangbaoWord::cutTextByRank(const string &text) {
  for(auto it = phoenixRank_.begin();it != phoenixRank_.end();it++) {
    ...
    for(auto pos: posAll) {
      ...
      auto elem = std::make_tuple(word,pos,word.size(),rankRepeat,rankorig,0);
...
void QiangbaoWord::getWordPrediction(const string &text) {
  ...
  boost::split(list_textRemain, textRemain, boost::is_space(),boost::algorithm::token_compress_on);
...
void QiangbaoWord::calcPrediction(const multimap<int,WordElement> &clearWordSeq) {
