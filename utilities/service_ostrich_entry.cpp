...
static const uint16_t iConstFetchAPIPortRangeMin = 41221;
static const uint16_t iConstFetchAPIPortRangeMax = 41225;
...
static const uint16_t iConstSaveAPIPortRangeMin = 41226;
static const uint16_t iConstSaveAPIPortRangeMax = 41230;
...
void train_ostrich_save_upd_main(void) {
  ...
  for (uint16_t port = iConstSaveAPIPortRangeMin;
    ...
    try {
      ...
      auto sock = std::make_shared<udp::socket>(*io_service, *ep);
      DUMP_VAR(port);
      savePort(port, "/watorvapor/wai.storage/conf/train.ostrich.save.api.json");
