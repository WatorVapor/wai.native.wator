...
void train_parrot_fetch_upd_main(void) {
  ...
  for (uint16_t port = iConstFetchAPIPortRangeMin;
    ...
    try {
      ...
      DUMP_VAR(port);
      savePort(port, "/watorvapor/wai.storage/conf/train.parrot.fetch.api.json");
...
static const uint16_t iConstSaveAPIPortRangeMin = 41234;
static const uint16_t iConstSaveAPIPortRangeMax = 41244;
...
void train_parrot_save_upd_main(void) {
  ...
  for (uint16_t port = iConstSaveAPIPortRangeMin;
    ...
    try {
      ...
      auto sock = std::make_shared<udp::socket>(*io_service, *ep);
      DUMP_VAR(port);
      savePort(port, "/watorvapor/wai.storage/conf/train.parrot.save.api.json");
